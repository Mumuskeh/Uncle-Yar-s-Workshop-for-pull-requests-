-Module:
Sig MCX LVAW v1 [Kmack]
Only released in Gamma Discord server.

----------------------------------------------------------
-Compatibility:
Note: this only will work for the first release of the mod, and not for the later update with .300 Blackout integration.

Disable the original configs folder.

----------------------------------------------------------
-Explanation:

Reworked the npc loadouts a bit. Only veterans and masters will carry it, veterans only in default configuration, and only the veterans of mercs, ISG and Monolith.
Edited text to clarify this is the 5.45mm model.
Moved both icons to the same file.

Integrated into:
Mags Redux
Weapon Parts Overahul

----------------------------------------------------------
-Patches:

Honey Badger and .300 BLK Framework [Pillii]
Adds a cloned variant chambered in .300 Blackout.

----------------------------------------------------------
-Known issues:
